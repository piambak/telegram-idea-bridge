...
// defaultParams merge under a caller's own `params` (e.g. groq's
// reasoning_effort below) — a per-call param always wins over the provider's
// default.
function makeOpenAICompatChat({ baseUrl, model, apiKeyEnv, getStartHelp, defaultParams }) {
	async function attempt(messages, timeoutMs, params) {
		...
		try {
			const res = await fetch(`${baseUrl}/chat/completions`, {
				...
				body: JSON.stringify({ model, messages, ...defaultParams, ...params }),
	...
	// message shape the callers already surface to Telegram. `params` (e.g.
	// temperature, max_tokens, response_format) rides along unchanged through
	// every retry attempt.
	return async function chat(messages, { timeoutMs = 2 * 60 * 1000, retries = 2, params = {} } = {}) {
		let lastErr;
...
const groqChat = makeOpenAICompatChat({
	...
	// Roughly halves latency on gpt-oss-120b (docs/AGENTS-SKILLS.md); a
	// caller can still ask for more reasoning via params.reasoning_effort.
	defaultParams: { reasoning_effort: 'low' },
});
...
const MODELS = {
	...
	openrouter: {
		label: 'OpenRouter (Nex N2.5 Pro, free) — confirmed working on this network',
		chat: openrouterChat,
	},
};
...
module.exports = {
	...
	modelForJob,
	parseModelOverride,
	makeOpenAICompatChat,
	// Raw single-provider chat fns, not the 'groq' alias's fallback chain —
	// /status (lib/status.js) needs to probe Groq/OpenRouter/Ollama Cloud
	// independently rather than through a call that silently moves on to
	// the next provider.
	groqChat,
	openrouterChat,
	ollamacloudChat,
