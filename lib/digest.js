...
// Bank/e-wallet sender or amount+transaction-wording detection, and the
// amount/type/account extraction itself, both live in lib/finance.js
// (finance.fromEmail) — the canonical ~40-domain list and Rupiah/rb/jt
// parser docs/V2-SPEC.md §3 names explicitly, not a second copy here.
// "Is this a finance email" and "build its Transaction" are the same
// question, so this rule doubles as the extraction: a truthy result IS the
// Transaction, reused directly in triage() below instead of re-deriving it.
function financeTransactionFor(msg) {
	return finance.fromEmail(msg);
}

function isFinance(msg) {
	return Boolean(financeTransactionFor(msg));
}
...
async function triage(messages, chat) {
	...
	messages.forEach((msg, i) => {
		const transaction = isMeeting(msg) ? null : financeTransactionFor(msg);
		if (isMeeting(msg)) {
			...
			results[i] = { message: msg, category: 'finance', summary: msg.subject, action: null, due: null, priority: 'medium', event: null, transaction };
...
//
// `logTransaction` defaults to finance.logTransaction (ref-based dedup
// against the email id, so re-running the digest never double-logs) but
// stays overridable/injectable for tests, same as pushEvent below.
async function autoActions(triaged, { logTransaction = finance.logTransaction, pushEvent = radicale.pushEvent } = {}) {
	const autoLog = env.FINANCE_AUTO_LOG === '1';
	const autoCalendar = env.DIGEST_AUTO_CALENDAR === '1';

	for (const item of triaged) {
		if (item.category === 'finance' && autoLog && item.transaction) {
			try {
				const result = await logTransaction(item.transaction);
				item.autoLogged = result ? result.logged !== false : true;
			} catch (err) {
