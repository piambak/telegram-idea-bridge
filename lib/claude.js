...
// Deep-tier entry point: invokes a skill by name, the way the CLI's own
// `/<skill> <args>` convention does, optionally with a large document on
// stdin. Still just one prompt string piped to stdin under the hood — args
// stays short (a title, a brief), the document (if any) rides along after
// it, so the whole thing is never an argv element regardless of size
// (docs/ANALYSIS.md bug #1).
function runSkill(name, args, opts = {}) {
	const { stdin, ...runOpts } = opts;
	const header = `/${name} ${args || ''}`.trim();
	const prompt = stdin ? `${header}\n\n${stdin}` : header;
	return run(prompt, runOpts);
}

// A cheap liveness check for /status — confirms the binary starts and
// exits 0, without spending a real session/prompt the way run() would.
function version() {
	const bin = env.CLAUDE_BIN || 'claude';
	const useShell = env.CLAUDE_SHELL === '1';
	return new Promise((resolve, reject) => {
		const child = cp.spawn(bin, ['--version'], { cwd: REPO_ROOT, shell: useShell, windowsHide: true });
		let stdout = '';
		let settled = false;
		const timer = setTimeout(() => {
			if (settled) return;
			settled = true;
			child.kill();
			reject(new Error('Claude CLI --version timed out'));
		}, 10_000);
		child.stdout.on('data', (chunk) => {
			stdout += chunk;
		});
		...
		child.on('close', (code) => {
			...
			settled = true;
			clearTimeout(timer);
			if (code !== 0) return reject(new Error(`Claude CLI --version exited ${code}`));
			resolve(stdout.trim());
		});
	});
}

module.exports = { run, runSkill, parseJsonLoose, version };
