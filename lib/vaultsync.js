...
// For /status: is the vault directory there, did the last sync succeed, and
// is the remote actually reachable right now (a real network round-trip,
// not just "the last push happened to work").
async function checkConnection() {
	if (!fs.existsSync(VAULT_DIR)) return { ok: false, reason: `vault directory not found: ${VAULT_DIR}` };
	const failure = getLastError();
	if (failure) return { ok: false, reason: `last sync failed: ${failure.message}` };
	try {
		await run(['ls-remote', '--exit-code', 'origin', 'HEAD']);
		return { ok: true };
	} catch (err) {
...
// Exposed as the app's one general-purpose mutex, not just for sync()'s own
// git commands: docs/V2-SPEC.md §6 requires every scheduled job (regcheck,
// reminder tick, inbox digest, monthly finance report) to be serialised
// against it too, so a slow job can never overlap a vault sync (or another
// job) touching the same working tree / .state files at once.
module.exports = { sync, serialized, VAULT_DIR, getLastError, checkConnection };
