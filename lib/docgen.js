...
async function draftContent(brief) {
	// The brief goes to the CLI's stdin, never argv (docs/ANALYSIS.md bug #1).
	const result = await claude.run(DRAFT_PROMPT(brief), { timeoutMs: 5 * 60 * 1000 });
	if (!result.ok) throw new Error(result.error || 'Claude CLI failed to draft the document');
	const reply = result.raw;
...
async function draftTable(brief) {
	// The brief goes to the CLI's stdin, never argv (docs/ANALYSIS.md bug #1).
	const result = await claude.run(EXCEL_PROMPT(brief), { timeoutMs: 5 * 60 * 1000 });
	if (!result.ok) throw new Error(result.error || 'Claude CLI failed to draft the table');
