...
// so this module never needs a Telegram dependency.
async function tick() {
	const list = loadAll();
	...
	for (const r of list) {
		...
		if (r.lastTaskId && r.lastTaskFor === r.nextDue) {
			...
			try {
				...
				console.error(`[reminders] failed to check task completion for #${r.id}:`, err.message);
			}
			if (completed) {
				rollForward(r);
				changed = true;
		...
		if (withinLead && !taskExistsThisCycle) {
			try {
				...
				console.error(`[reminders] failed to create task for #${r.id}:`, err.message);
			}
